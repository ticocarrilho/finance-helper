...
describe('GET /api/card', () => {
  it('should be able to get all its cards', async () => {
    ...
    await factory.createMany('Card', 5, {
      user_id: userA.id
    });
    await factory.createMany('Card', 2, {
      user_id: userB.id
    });
  ...
  it('an user should be able to get its card', async () => {
  ...
  it('an user should not be able to get someone else`s card', async () => {
    const userA = await factory.create('User', {
      email: 'email@gmail.com',
      password: '12345678'
    });
  ...
  it('should return 404 when trying to get a card that does not exists', async () => {
    const user = await factory.create('User', {
      password: '12345678'
    });
